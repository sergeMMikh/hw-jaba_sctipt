# Домашнее задание к занятию «1.4. JavaScript»- Михалёв Сергей
...
<details>
<summary>оставил "на сладкое"</summary>
...
Передал в строке "Имя" строку `<script>alert('pwned')</script>`. Данный код сохраняется на сервере и впоследствии выполняется в браузере другого пользователя при открытии страницы.

Может ли трекер быть причастен к происшествию с клиентами? **Да, может:**
**- система принимает и хранит JS-код**
**- этот код выполняется у других пользователей**

Это означает, что злоумышленник может внедрить произвольный JavaScript-код, который будет выполняться у всех пользователей, просматривающих страницу. Такой код может:

* считывать cookies и сессионные данные пользователей;
* извлекать информацию со страницы (например, имена,  комментарии и другие данные);
* отправлять полученные данные на внешний сервер;
выполнять действия от имени пользователя.

Таким образом, трекер может использоваться для передачи дополнительной информации, не ограничиваясь заявленным функционалом (например, временем работы), поскольку внедрённый JavaScript способен отправлять любые данные с клиентской стороны.
